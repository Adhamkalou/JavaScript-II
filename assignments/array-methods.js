...
// Problem 1
let runningNumberEmail = [];
runners.forEach(currentValue =>
  runningNumberEmail.push(`${currentValue.id} ${currentValue.email}`));
console.log(runningNumberEmail);

// Problem 2
let powerRampage = runners.map(currentValue =>
    `${currentValue.first_name.toLowerCase()} ${currentValue.last_name.toUpperCase()}`);
  console.log(powerRampage);

// Problem 3
let callSheet = [];
runners.forEach(currentValue =>
  callSheet.push(`${currentValue.last_name}, ${currentValue.first_name}`));
callSheet.sort();
